...
const Adddormitory: React.FC = () => {
  ...
  const handleSubmit = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!formData.phone.trim()) newErrors.phone = 'จำเป็นต้องกรอกเบอร์โทรศัพท์';
    if (formData.dueDate === '') newErrors.dueDate = 'จำเป็นต้องระบุวันสุดท้ายของการชำระเงิน';
    if (formData.finePerDay === '') newErrors.finePerDay = 'จำเป็นต้องระบุค่าปรับ';
      ...
      <div className="flex-grow w-full max-w-6xl mx-auto px-6 py-10 flex flex-col items-center">
        ...
        <div className="w-full flex justify-end px-4 md:px-12">
            <button 
                ...
                className="bg-[#7d7671] hover:bg-[#635d59] text-white px-10 py-2 rounded-md font-medium shadow transition-colors"
            >
                สร้าง
            </button>
        </div>
